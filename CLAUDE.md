...
- `pnpm --filter @workspace/scripts run icons` — re-rasterize the PWA icons from
  their SVG sources (needs a chrome-headless-shell binary, see `docs/notes/pwa.md`)
...
| `artifacts/safarly/src/lib/pwa.ts` | Install-prompt eligibility rules (pure) — the Chromium vs iOS split, plus the deferred-prompt stash |
| `artifacts/safarly/src/components/PWAProvider.tsx` | Offline banner, update prompt and install card for the installed app |
| `artifacts/safarly/public/icons/` | PWA icon SVG sources + the committed PNGs the manifest points at |
...
- **The app installs as a PWA, not a native shell.** The same React build that
  serves the website is the mobile app: a web manifest plus a Workbox service
  worker generated by `vite-plugin-pwa` (configured inline in `vite.config.ts`).
  Chosen over Capacitor and over a React Native rewrite because the whole UI
  layer carries over unchanged and nothing needs store accounts or signing keys.
  The service worker precaches the shell **and every lazy route chunk**, so the
  locally-generated itinerary, the phrasebook and destination browsing all work
  with no connection — while the three API-backed features (enrichment,
  Concierge, Live Lens) are deliberately left uncached so they fail the way they
  already fail rather than serving a stale answer. See `docs/notes/pwa.md`.
...
- `pwa.test.ts` — install-prompt eligibility: the already-installed case, the 30-day
  snooze (including a clock-skewed dismissal, which a naive comparison hides forever),
  the iPadOS-reports-as-a-Mac detection, and the single-use deferred-prompt stash.
...
- **The fixed chrome heights are tokens now, not literals.** `--sf-navbar-h` and
  `--sf-bottomnav-h` in `index.css` replaced the bare `68`/`88` that eleven files
  each repeated. They exist because `index.html` opted into `viewport-fit=cover`
  for the installed app, which lets the page paint under the notch and makes the
  real clearance device-dependent. Off a notched device the insets are `0px` and
  the tokens collapse to exactly the numbers they replaced — so a new page that
  hardcodes `68` looks fine everywhere except the case the tokens exist for.
